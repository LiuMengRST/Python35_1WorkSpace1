...
class PTBModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        def lstm_cell():
            return tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=True)
        attn_cell = lstm_cell
        if is_training and config.keep_prob<1:
            def attn_cell():
                return tf.nn.rnn_cell.MultiRNNCell(lstm_cell(), out_keep_prob=config.keep_prob)
        ...
        loss = sequence_loss_by_example([logits], [tf.reshape(input_.targets,[-1])],[tf.ones([batch_size * num_steps],dtype=tf.float32)])
